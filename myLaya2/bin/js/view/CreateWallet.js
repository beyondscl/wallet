...
(function (view) {
    var CreateWallet = /** @class */ (function (_super) {
        ...
        CreateWallet.prototype.goBack = function () {
            ...
            this.parentUI == null ? new view.EnterApp() : new view.WalletMain().initQueryData(testData.getWalletInfo(this.parentUI.lab_wName.text)); //获取数据
        };
        ...
        CreateWallet.prototype.createWallet = function () {
            if (this.checkArgs()) {
                Laya.stage.removeChild(this.comp);
                //创建重复未判定
                var wallet = new mod.walletMod(this.comp.text_wall_name.text, this.comp.text_pass.text, "privatekey", "keystore", "851a3sdf18851a3sdf18851a3sdf18851a3sdf1852", ['ETH']);
                var walletString = wallet.toJson();
                laya.net.LocalStorage.setItem(this.comp.text_wall_name.text, JSON.stringify(walletString));
                var appStore = laya.net.LocalStorage.getItem(config.prod.appKey);
                if (appStore) {
                    var walletOld = JSON.parse(appStore);
